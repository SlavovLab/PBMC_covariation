########################################################################################
## Processing raw data and quality control for proteomic dataset: n6p1
########################################################################################

stats <- diann_load("/projects/slavov/LK/ImmPTR/data/raw_data/n6p1/diann_b17/report.stats.tsv")
linker <- stats %>% dplyr::select(File.Name)
linker <- linker %>% filter(!grepl("d8", File.Name))
linker <- linker %>%
  mutate(
    Run = gsub(".*\\\\|\\.d$", "", File.Name),
    Well = gsub(".*sc_R([A-Z0-9]+)_.*", "\\1", Run)
  )
linker$plate <- "1"
...
data_path <- '/projects/slavov/LK/ImmPTR/data/raw_data/n6p1/diann_b17/report.tsv'
link_path <- '/projects/slavov/LK/PBMC_PTR/datasets/n6p1/meta_data/meta_n6p1.csv'
## Read in cell isolation files from CellenONE 
two <- '/projects/slavov/LK/ImmPTR/data/meta_data/n6p1/don5_sorted2_use_isolated2.xls'
one <- '/projects/slavov/LK/ImmPTR/data/meta_data/n6p1/don4_sorted1_use_isolated2.xls'
all_cells <- list(Donor05 = two,
                  Donor04 = one)
...
### A bug in this particular cellenONE mapping sequence --> manually remove sample ID A11.0 with diameter = 10.56
### the correct sample ID for A11.0 indeed has diameter 12.36 based on manually inspecting the raw cellenONE data sorting file
### Also the same was done for A21.4 which correct diameter from the raw data is 10.00, not 14.14. 
meta.data <- meta.data[!(meta.data$ID == "A11.0" & meta.data$diameter == 10.56), ]
meta.data <- meta.data[!(meta.data$ID == "A21.4" & meta.data$diameter == 14.14), ]

rep <- rep.full
rep <- rep %>% filter(Lib.Q.Value <= 0.01, Lib.PG.Q.Value <= 0.05, Ms1.Area > 0, !grepl("d8_MBR|d8MBR", Run), grepl("HUMAN", Protein.Names)) 
...
pep_PG_map <- rep %>% dplyr::select(Stripped.Sequence, seqcharge, Protein.Group, Genes) %>% distinct()
...
filtered_IDsSignal_CellSelection <- ggplot(CTQ_filtered_raw_data_stats_NOCARRIER, aes(y = log10(total_MS1_signal), x = precursor_counts)) +
  ...
  labs(title = "Single Cell Selection by IDs and Signal (n6p1)",
...
filtered_plot_CV_CellSelection <- ggplot(median_CV_df, aes(x = Median_CV, fill = SampleType)) +
  ...
  labs(title = "Single Cell Selection by CV (n6p1)",
...
prot_matrix_n6p1 <- sc.batch_cor
prot_df_n6p1 <- reshape2::melt(prot_matrix_n6p1)
colnames(prot_df_n6p1) <- c("Genes", "ID", "Prot_FC_log2")
cellID_donorID <- meta.data %>% dplyr::select(sample, ID) %>% distinct()
prot_df_n6p1 <- prot_df_n6p1 %>% left_join(cellID_donorID, by=c("ID"="ID"))
prot_df_n6p1$dataset_batch <- "n6p1"
prot_df_n6p1$ID <- paste0(prot_df_n6p1$ID, "_", prot_df_n6p1$dataset_batch)


### Save a list of selected cell IDs that passed quality control

fwrite(prot_df_n6p1,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Genes_n6p1_for_prot_integration.csv",
       row.names = FALSE)

prot_matrix_n6p1.colonly <- sc.batch_cor.colonly
prot_df_n6p1.colonly <- reshape2::melt(prot_matrix_n6p1.colonly)
colnames(prot_df_n6p1.colonly) <- c("Genes", "ID", "Prot_FC_log2")
prot_df_n6p1.colonly <- prot_df_n6p1.colonly %>% left_join(cellID_donorID, by = c("ID" = "ID"))
prot_df_n6p1.colonly$dataset_batch <- "n6p1"
prot_df_n6p1.colonly$ID <- paste0(prot_df_n6p1.colonly$ID, "_", prot_df_n6p1.colonly$dataset_batch)
fwrite(prot_df_n6p1.colonly,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Genes_n6p1_for_abundance_analysis.csv",
       row.names = FALSE)
...
pep_prot_map_n6p1 <- rep.selected.d04 %>%
  dplyr::select(Stripped.Sequence, Genes, Protein.Group) %>%
  distinct()

pep_prot_map_n6p1 <- pep_prot_map_n6p1 %>%
...
proteotypic_flag <- pep_prot_map_n6p1 %>%
...
pep_prot_map_n6p1$dataset_batch <- "n6p1"

### Long-format peptide outputs, parallel to prot_df_n6p1 / prot_df_n6p1.colonly
...
peptide_df_n6p1 <- reshape2::melt(sc.batch_cor.pep)
colnames(peptide_df_n6p1) <- c("Stripped.Sequence", "ID", "Peptide_FC_log2")
peptide_df_n6p1 <- peptide_df_n6p1 %>% left_join(cellID_donorID_pep, by = "ID")
peptide_df_n6p1$dataset_batch <- "n6p1"
peptide_df_n6p1$ID <- paste0(peptide_df_n6p1$ID, "_", peptide_df_n6p1$dataset_batch)

# Absolute-friendly (column-only normalized, ComBat plex + limma Order corrected)
peptide_df_n6p1.colonly <- reshape2::melt(sc.batch_cor.pep.colonly)
colnames(peptide_df_n6p1.colonly) <- c("Stripped.Sequence", "ID", "Peptide_FC_log2")
peptide_df_n6p1.colonly <- peptide_df_n6p1.colonly %>% left_join(cellID_donorID_pep, by = "ID")
peptide_df_n6p1.colonly$dataset_batch <- "n6p1"
peptide_df_n6p1.colonly$ID <- paste0(peptide_df_n6p1.colonly$ID, "_", peptide_df_n6p1.colonly$dataset_batch)

### Save outputs
fwrite(peptide_df_n6p1,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Peptides_n6p1_for_integration.csv",
       row.names = FALSE)
fwrite(peptide_df_n6p1.colonly,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Peptides_n6p1_for_abundance_analysis.csv",
       row.names = FALSE)
fwrite(pep_prot_map_n6p1,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/PepProtMap_n6p1.csv",
       row.names = FALSE)

### Save a list of selected cell IDs that passed quality control

IdentificationsList_n6p1 <- rep.selected.d04 %>% distinct()

fwrite(IdentificationsList_n6p1,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/IdentificationsList_n6p1.csv",
       row.names = FALSE)
...
Raw_data_with_spike_in <- rep.full %>% filter(Lib.Q.Value <= 0.01, Lib.PG.Q.Value <= 0.05, Ms1.Area > 0, !grepl("d8_MBR|d8MBR", Run)) 
sel.ID.diameters <- meta.data %>% dplyr::select(diameter, ID)
...
RobustRegression_SpikeIns_n6p1 <- ggplot(sum_stats_spike_ins, 
...
SingleCell_Versus_SpikeIn_Abundances_n6p1 <- ggplot(rep_analyze_spike_ins, 
