########################################################################################
## Processing raw data and quality control for proteomic dataset: n6p2
########################################################################################

stats <- diann_load("/projects/slavov/LK/ImmPTR/data/raw_data/n6p2/diann_b17/report.stats.tsv")
...
data_path <- '/projects/slavov/LK/ImmPTR/data/raw_data/n6p2/diann_b17/report.tsv'
link_path <- '/projects/slavov/LK/PBMC_PTR/datasets/n6p2/meta_data/meta_n6p2.csv'
## Read in cell isolation files from CellenONE 
two <- '/projects/slavov/LK/ImmPTR/data/meta_data/n6p2/don5_sorted2_use_isolated2.xls'
one <- '/projects/slavov/LK/ImmPTR/data/meta_data/n6p2/don4_sorted1_use_isolated2.xls'
...
filtered_IDsSignal_CellSelection <- ggplot(CTQ_filtered_raw_data_stats_NOCARRIER, aes(y = log10(total_MS1_signal), x = precursor_counts)) +
  ...
  labs(title = "Single Cell Selection by IDs and Signal (n6p2)",
...
filtered_plot_CV_CellSelection <- ggplot(median_CV_df, aes(x = Median_CV, fill = SampleType)) +
  ...
  labs(title = "Single Cell Selection by CV (n6p2)",
...
prot_matrix_n6p2 <- sc.batch_cor
prot_df_n6p2 <- reshape2::melt(prot_matrix_n6p2)
colnames(prot_df_n6p2) <- c("Genes", "ID", "Prot_FC_log2")
cellID_donorID <- meta.data %>% dplyr::select(sample, ID) %>% distinct()
prot_df_n6p2 <- prot_df_n6p2 %>% left_join(cellID_donorID, by=c("ID"="ID"))
prot_df_n6p2$dataset_batch <- "n6p2"
prot_df_n6p2$ID <- paste0(prot_df_n6p2$ID, "_", prot_df_n6p2$dataset_batch)


### Save a list of selected cell IDs that passed quality control

fwrite(prot_df_n6p2,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Genes_n6p2_for_prot_integration.csv",
       row.names = FALSE)

# Save parallel output CSV
prot_matrix_n6p2.colonly <- sc.batch_cor.colonly
prot_df_n6p2.colonly <- reshape2::melt(prot_matrix_n6p2.colonly)
colnames(prot_df_n6p2.colonly) <- c("Genes", "ID", "Prot_FC_log2")
prot_df_n6p2.colonly <- prot_df_n6p2.colonly %>% left_join(cellID_donorID, by = c("ID" = "ID"))
prot_df_n6p2.colonly$dataset_batch <- "n6p2"
prot_df_n6p2.colonly$ID <- paste0(prot_df_n6p2.colonly$ID, "_", prot_df_n6p2.colonly$dataset_batch)
fwrite(prot_df_n6p2.colonly,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Genes_n6p2_for_abundance_analysis.csv",
       row.names = FALSE)
...
pep_prot_map_n6p2 <- rep.selected.d04 %>%
  dplyr::select(Stripped.Sequence, Genes, Protein.Group) %>%
  distinct()

pep_prot_map_n6p2 <- pep_prot_map_n6p2 %>%
...
proteotypic_flag <- pep_prot_map_n6p2 %>%
...
pep_prot_map_n6p2$dataset_batch <- "n6p2"

cellID_donorID_pep <- sample_map %>% dplyr::select(sample, ID) %>% distinct()

peptide_df_n6p2 <- reshape2::melt(sc.batch_cor.pep)
colnames(peptide_df_n6p2) <- c("Stripped.Sequence", "ID", "Peptide_FC_log2")
peptide_df_n6p2 <- peptide_df_n6p2 %>% left_join(cellID_donorID_pep, by = "ID")
peptide_df_n6p2$dataset_batch <- "n6p2"
peptide_df_n6p2$ID <- paste0(peptide_df_n6p2$ID, "_", peptide_df_n6p2$dataset_batch)

peptide_df_n6p2.colonly <- reshape2::melt(sc.batch_cor.pep.colonly)
colnames(peptide_df_n6p2.colonly) <- c("Stripped.Sequence", "ID", "Peptide_FC_log2")
peptide_df_n6p2.colonly <- peptide_df_n6p2.colonly %>% left_join(cellID_donorID_pep, by = "ID")
peptide_df_n6p2.colonly$dataset_batch <- "n6p2"
peptide_df_n6p2.colonly$ID <- paste0(peptide_df_n6p2.colonly$ID, "_", peptide_df_n6p2.colonly$dataset_batch)

### Save outputs
fwrite(peptide_df_n6p2,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Peptides_n6p2_for_integration.csv",
       row.names = FALSE)
fwrite(peptide_df_n6p2.colonly,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/Peptides_n6p2_for_abundance_analysis.csv",
       row.names = FALSE)
fwrite(pep_prot_map_n6p2,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/PepProtMap_n6p2.csv",
       row.names = FALSE)


### Save a list of selected cell analyte IDs that passed quality control


IdentificationsList_n6p2 <- rep.selected.d04 %>% distinct()

fwrite(IdentificationsList_n6p2,
       file = "/projects/slavov/LK/ImmPTR/results/final_scripts_output/IdentificationsList_n6p2.csv",
       row.names = FALSE)
...
RobustRegression_SpikeIns_n6p2 <- ggplot(sum_stats_spike_ins, 
...
SingleCell_Versus_SpikeIn_Abundances_n6p2 <- ggplot(rep_analyze_spike_ins, 
